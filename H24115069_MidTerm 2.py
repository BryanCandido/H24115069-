...
def add_obstacles(maze, num_obstacles):
    ...
    for row, col in [(i, j) for i in range(N) for j in range(M) if maze[i][j] == 1]:
        obstacles = 0
    while obstacles<num_obstacles:
        row,col = random.radint(0,N+1), random.radint(M+1,0)
    if (row,col) : 
        not path and maze[row][col] == 1
        maze[row][col] == 2
        obstacles += 1
    return maze
